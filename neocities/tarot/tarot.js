...
function flipCard(card) {
	...
	card.animate({transform: ["scaleX(1)", "scaleX(0)"]}, {duration: 1000, easing: "ease-in"}).finished.then(() => {
		...
		for (const letter of generateCardName()) {
			let node;
			if ("efghilnoprstuvw".includes(letter)) node = document.createElement("img"), node.src = "_assets/letters/" + letter + ".png", node.alt = letter;
			else if (letter == " ") node = document.createElement("span"), node.className = "space";
			else if (letter == ".") node = document.createElement("img"), node.src = "_assets/letters/period.png", node.alt = ".";
