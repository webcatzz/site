...
3. use the script! here's an example setup:
	...
	<script src="wherever/youre/storing/the/audio-player-script.js"></script>
	<script>
	AudioPlayer.create([
		// playlist
...
if you're looking for more functionality:

	here's a list of all possible settings. each one is optional!
		playButton
	...
	adding your own functionality? here's a list of all of AudioPlayer's properties:
		audio
			the audio object playing the track
		list
			an array of objects representing the playlist
		idx
			the current index in the playlist
		on(event, func)
			runs addEventListener(event, func) on the audio object
		next()
...
const AudioPlayer = {
	...
	create: function (list, options) {
		...
		if (options.playButton) {
			options.playButton.ariaLabel = "play";
		...
		if (options.skipButton) {
			options.skipButton.ariaLabel = "skip";
		...
		if (options.slider) {
			...
			this.slider.role = "slider", this.slider.ariaValueMin = 0, this.slider.tabIndex = 0;
			...
			this.on("timeupdate", () => {
				...
				this.alertSlider(this.audio.currentTime / this.audio.duration);
			});

			this.sliderMove = e => this.alertSlider(Math.max(0, Math.min(this.slider.clientWidth, e.x - this.sliderX)) / this.slider.clientWidth);
			this.sliderUp = e => {
				removeEventListener("mousemove", this.sliderMove);
				this.audio.currentTime = this.audio.duration * Math.max(0, Math.min(this.slider.clientWidth, e.x - this.sliderX)) / this.slider.clientWidth;
				delete this.sliderX;
			}
